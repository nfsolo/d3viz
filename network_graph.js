...
/*var myColor = d3.scaleOrdinal()
  .domain(gender_options)
  .range(d3.schemeSet2);

// color for legend
var colorl = d3.scaleQuantile()
            .range(["#eff3ff","#bdd7e7","#6baed6","#08519c"]);
*/
...
//-----------------------------DATA------------------------------//
dataset = d3.csv("data/sen_edges_fin.csv");
...
dataset_metadata.then(function(data) {
  ...
  dataset_topics.then(function(data) {
      ...
      links.forEach(function(link) {
          link.source = nodes[link.sen_a] || (nodes[link.sen_a] = {name: link.sen_a});
          link.target = nodes[link.sen_b] || (nodes[link.sen_b] = {name: link.sen_b});
      });
      ...
      for (let key in nodes){
        var result = metadata.filter(function(sen_meta) {
          return key === sen_meta.tweet_screen_name;
        });
    ...
    //---------- add links ----------------//
        ...
        links.forEach(function(link) {
           for (var i = 0; i < highlight_node_count; i++) {
                if (link.sen_a == most_influential_congress_handles[i] || link.sen_b == most_influential_congress_handles[i]){
                  link.source = nodes[link.sen_a] || (nodes[link.sen_a] = {name: link.sen_a});
                  link.target = nodes[link.sen_b] || (nodes[link.sen_b] = {name: link.sen_b});
      ...
      colorl.domain([min_degree, max_degree]);
  ...
  if (dropdown == "gender"){
    if (selectedGroup == "All"){
      all_links.forEach(function(link) {
         for (var i = 0; i < highlight_node_count; i++) {
            if (link.sen_a == most_influential_congress_handles[i] || link.sen_b == most_influential_congress_handles[i]){
              ...
              if (curr_party != "All"){
                if(all_nodes[link.sen_a].party != curr_party || all_nodes[link.sen_b].party != curr_party){
                  continue;
                }
              }
              if (curr_topic != "All"){
                if(all_nodes[link.sen_a].topic != curr_topic || all_nodes[link.sen_b].topic != curr_topic){
                  continue;
                }
              }
              link.source = nodes[link.sen_a] || (nodes[link.sen_a] = {name: link.sen_a});
      ...
      all_links.forEach(function(link) {
         for (var i = 0; i < highlight_node_count; i++) {
            if (link.sen_a == most_influential_congress_handles[i] || link.sen_b == most_influential_congress_handles[i]){
              if (all_nodes[link.sen_a].gender == selectedGroup && all_nodes[link.sen_b].gender == selectedGroup){
                //if the party or topic of the node is incorrect, skip that node
                if (curr_party != "All"){
                  if(all_nodes[link.sen_a].party != curr_party || all_nodes[link.sen_b].party != curr_party){
                    continue;
    ...
    if (selectedGroup == "All"){
      all_links.forEach(function(link) {
         for (var i = 0; i < highlight_node_count; i++) {
            if (link.sen_a == most_influential_congress_handles[i] || link.sen_b == most_influential_congress_handles[i]){
              ...
              if (curr_gender != "All"){
                if(all_nodes[link.sen_a].gender != curr_gender || all_nodes[link.sen_b].gender != curr_gender){
                  continue;
                }
              }
              if (curr_topic != "All"){
      ...
      all_links.forEach(function(link) {
        for (var i = 0; i < highlight_node_count; i++) {
          if (link.sen_a == most_influential_congress_handles[i] || link.sen_b == most_influential_congress_handles[i]){
            if (all_nodes[link.sen_a].party == selectedGroup && all_nodes[link.sen_b].party == selectedGroup){
              //if the gender or topic of the node is incorrect, skip that node
              if (curr_gender != "All"){
                if(all_nodes[link.sen_a].gender != curr_gender || all_nodes[link.sen_b].gender != curr_gender){
                  continue;
    ...
    if (selectedGroup == "All"){
      ...
      all_links.forEach(function(link) {
        for (var i = 0; i < highlight_node_count; i++) {
          if (link.sen_a == most_influential_congress_handles[i] || link.sen_b == most_influential_congress_handles[i]){
            if (all_nodes[link.sen_a].topic == selectedGroup && all_nodes[link.sen_b].topic== selectedGroup){
              //if the gender or party of the node is incorrect, skip that node
              if (curr_gender != "All"){
                if(all_nodes[link.sen_a].gender != curr_gender || all_nodes[link.sen_b].gender != curr_gender){
                  continue;
